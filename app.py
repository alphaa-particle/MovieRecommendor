...
try:
    ...
    st.error("Model files not found. Please make sure 'movie_dict.pkl' and 'similarity.pkl' are in the same directory.")
    st.stop()


# --- API FETCH FUNCTIONS ---

@st.cache_data(ttl=3600) # Cache posters for 1 hour
def get_background_posters():
    """Fetches posters for the dynamic background from TMDB's popular movies."""
    try:
...
def fetch_movie_details(movie_id, movie_title):
    """Fetches extensive movie details, trying TMDB first, then OMDB as a fallback."""
    ...
    try:
        ...
        data = requests.get(url).json()
        if 'id' in data and data.get('poster_path'): # Prioritize TMDB if it has a poster
            ...
            reviews = data.get('reviews', {}).get('results', []) # Get the full list of reviews
            return poster, overview, release_date, rating, genres, tagline, runtime, revenue, reviews
    except Exception:
        pass # If TMDB fails, we'll try OMDB next
    ...
    try:
        ...
        if data.get("Response") == "True":
            ...
            return poster, overview, release_date, rating, genres, "", runtime, 0, [] # Return empty list for reviews
    except Exception:
        pass # If both fail, return placeholder data

    return "https://via.placeholder.com/500x750.png?text=Info+Not+Found", "Could not fetch details.", "N/A", 0, [], "", 0, 0, []


# --- RECOMMENDATION LOGIC ---

def recommend(movie):
    """Finds the top 10 most similar movies and returns their details."""
    try:
        # Find the index of the selected movie
        movie_index = movies[movies['title'] == movie].index[0]
        # Get the similarity scores for that movie
        distances = similarity[movie_index]
        # Sort by similarity and get the top 10 (index 1 to 11 to exclude the movie itself)
        ...
        for i in movies_list:
            ...
            recommended_movies_data.append({
                ...
                "tagline": details[5],
                "runtime": details[6],
                "reviews": details[8] # Pass the full list of reviews
            })
        return recommended_movies_data
    except IndexError:
        st.error("Movie not found in the dataset. This can happen with duplicate titles. Please select another movie.")
        return []
...
<style>
    ...
    @keyframes vault-unlock {{ 0% {{ stroke-dashoffset: 283; }} 50% {{ stroke-dashoffset: 0; }} 100% {{ stroke-dashoffset: -283; }} }}
    @keyframes inner-spin {{ from {{ transform: rotate(0deg); }} to {{ transform: rotate(360deg); }} }}
    /* --- General Styling --- */
    .stApp {{
        background-color: rgba(43, 43, 43, 0.85);
        backdrop-filter: blur(10px); color: #F5F5DC; font-family: 'Raleway', sans-serif;
    }}
    .stApp::after {{ /* Dark overlay for readability */
        content: ''; position: fixed; top: 0; left: 0; width: 100%; height: 100%;
        background: radial-gradient(circle, rgba(43,43,43,0.6) 0%, rgba(43,43,43,0.9) 100%); z-index: -1;
    }}
    /* --- Header Styling --- */
    ...
    .stTextInput > div > div > input {{
        background-color: rgba(54, 54, 54, 0.8); color: #F5F5DC; border: 1px solid #555555;
        border-radius: 10px; padding: 1rem; transition: all 0.3s ease;
    }}
    ...
    .vault-circle {{ stroke: #D4AF37; stroke-width: 5; fill: transparent; stroke-dasharray: 283; animation: vault-unlock 2s infinite linear; }}
    .inner-circle {{ stroke: #555555; stroke-width: 15; fill: transparent; animation: inner-spin 4s infinite linear; }}
    ...
    .genre-tag {{
        background-color: #555555; color: #F5F5DC; padding: 5px 12px; border-radius: 15px;
        font-size: 12px; margin-right: 5px; display: inline-block; margin-top: 5px;
    }}
    .review-box {{
        background-color: rgba(43, 43, 43, 0.8); border-left: 3px solid #D4AF37;
        padding: 15px; border-radius: 8px; margin-top: 1rem;
    }}
...
if search_term:
    ...
    if filtered_movies:
        selected_movie = st.selectbox("Select your movie from the search results:", filtered_movies, index=None, placeholder="Select a movie...")
    else:
        st.warning("No movies found with that name. Try another search.")
        selected_movie = None # Ensure selected_movie is None if no results
else:
    ...
    for message in loading_messages:
        loader_placeholder.markdown(f"""
            <div class="loader-container">
                ...
                <p class="loader-text" style="font-size: 0.9rem;">Fetching from over 7000+ movies for you...</p>
            </div>
        """, unsafe_allow_html=True)
        time.sleep(0.8) # Adjust timing as needed
    
    # Get recommendations and clear the loader
        ...
        for i, movie in enumerate(recommendations):
            with st.container():
                ...
                with col2:
                    ...
                    st.markdown(f"""<p class="movie-stats" style="margin-top: 1rem;"><strong>⏳ Runtime:</strong> {movie['runtime']} min</p>""", unsafe_allow_html=True)
                    ...
                    if movie['reviews']:
                        ...
                        for review in movie['reviews'][:2]: # Loop through the first 2 reviews
                            author = review.get('author', 'Unknown')
                            content = review.get('content', '')
                            # Truncate long reviews
                            content_snippet = content[:400] + '...' if len(content) > 400 else content
                            st.markdown(f"""<div class="review-box"><p class="review-author">Review by {author}:</p><p class="review-content">"{content_snippet}"</p></div>""", unsafe_allow_html=True)
                
                st.markdown('</div>', unsafe_allow_html=True)

# Handle case where button is clicked but no movie is selected
elif 'rec_button' in st.session_state and st.session_state.rec_button:
